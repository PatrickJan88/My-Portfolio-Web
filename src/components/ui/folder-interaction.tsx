...
export function FolderInteraction({
  ...
  const [isClicked, setIsClicked] = useState(defaultOpen);

  const isOpen = isHovered !== undefined ? (isHovered || isClicked) : isClicked;
  ...
  return (
    <div className={`w-full flex justify-center items-center select-none ${className}`}>
      <div
        onClick={(e) => {
          e.stopPropagation();
          setIsClicked((prev) => !prev);
        }}
        className="w-80 h-52 relative wrapper cursor-pointer group [perspective:1000px]"
      >
        <div
          className="folder relative w-[87.5%] mx-auto items-center h-full flex justify-center backdrop-blur-xl border border-white/25"
          style={{
            background:
              "linear-gradient(135deg, rgba(255, 255, 255, 0.22) 0%, rgba(255, 255, 255, 0.08) 100%)",
            boxShadow:
              "0px 0px 18px 2px rgba(255, 255, 255, 0.22) inset, 0 12px 30px rgba(0, 0, 0, 0.25)",
            borderRadius: 14,
          }}
        >
          {pages.map((page, i) => (
            ...
            >
              <Page title={page.title} accent={page.accent} />
            </motion.div>
          ))}
        </div>
        ...
        <motion.div
          ...
          className="absolute -left-[1px] -right-[1px] -bottom-[1px] z-20 h-44 rounded-3xl origin-bottom flex justify-center items-center overflow-visible pointer-events-none drop-shadow-[0_12px_24px_rgba(0,0,0,0.25)]"
        >
          <svg
...
const Page = ({ title, accent }: PageProps) => (
  <div className="w-full h-full bg-[#F6F6F6] rounded-xl shadow-lg p-3 sm:p-3.5 border border-neutral-200/60">
    <div className="flex flex-col gap-1.5 sm:gap-2">
      <div className="flex items-center justify-between pb-0.5">
        <div className={`w-3.5 h-1.5 rounded-full ${accent || "bg-[#A7A3C4]"}`} />
        {title && (
          <span className="text-[7.5px] font-semibold tracking-tight text-neutral-600 uppercase font-mono">
            {title}
          </span>
        )}
      </div>
      <div className="w-full h-1 sm:h-1.5 bg-neutral-300/80 rounded-full" />
      {Array.from({ length: 6 }).map((_, i) => (
        <div key={i} className="flex gap-1.5 sm:gap-2">
          <div className="flex-1 h-1 sm:h-1.5 bg-neutral-200/90 rounded-full" />
          <div className="flex-1 h-1 sm:h-1.5 bg-neutral-200/90 rounded-full" />
        </div>
      ))}
    </div>
