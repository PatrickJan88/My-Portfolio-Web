...
export interface ParticleScrollOptions {
  ...
  smoothing?: number;
  /** Optional programmatic progress (0: assembled, 1: fully dissolved into particles) */
  progress?: number;
}
...
export interface ParticleScrollInstance {
  ...
  resize: () => void;
  /** Force a repaint/resample of content */
  repaint: () => void;
...
void main () {
  ...
  outColor = vec4(mix(uBg, tex.rgb, vis * tex.a), vis * tex.a * uCover);
}`;
...
export function createParticleScroll(
  ...
  const sourceCtx = source.getContext("2d", { willReadFrequently: true }) as ElementImageContext | null;
  const paintable = source as PaintableCanvas;
  const hasNativeHtmlInCanvas = Boolean(
    sourceCtx &&
  ...
  function fallbackRasterize() {
    ...
    // Render child text/elements onto source canvas
    const textNodes: Array<{ text: string; el: HTMLElement; style: CSSStyleDeclaration; rect: DOMRect }> = [];
    const walk = (node: Element) => {
      ...
      // Check if it's a leaf node or has text
      if (el.children.length === 0 && el.textContent?.trim()) {
        textNodes.push({ text: el.textContent.trim(), el, style, rect });
      } else {
    ...
    textNodes.forEach(({ text, style, rect }) => {
      const x = rect.left - parentRect.left;
      const y = rect.top - parentRect.top;
      sourceCtx.font = `${style.fontWeight} ${style.fontSize} ${style.fontFamily}`;
      sourceCtx.fillStyle = style.color || "#e4e6ea";
      sourceCtx.textBaseline = "top";
      sourceCtx.textAlign = (style.textAlign as CanvasTextAlign) || "left";

      if (style.letterSpacing && style.letterSpacing !== "normal") {
        try {
          (sourceCtx as unknown as { letterSpacing: string }).letterSpacing = style.letterSpacing;
        } catch {}
      }

      if (style.textAlign === "center") {
        sourceCtx.fillText(text, x + rect.width / 2, y);
      } else if (style.textAlign === "right") {
        sourceCtx.fillText(text, x + rect.width, y);
      } else {
  ...
  function syncCanvasSize() {
    ...
    if (hasNativeHtmlInCanvas && paintable.requestPaint) {
  ...
  function rowTargetFor(docRowY: number) {
    ...
    // If programmatic progress is provided, calculate row target directly
    if (config.progress !== undefined && config.progress > 0) {
      const h = Math.max(output.clientHeight, 1);
      const rowNorm = docRowY / h;
      // Stagger dissolve across the height
      const dissolveThreshold = config.progress * 1.5 - rowNorm * 0.5;
      return Math.min(Math.max(1.0 - dissolveThreshold, 0), 1);
    }
  ...
  function updateRows(
    ...
    const docRows = Math.max(1, Math.ceil(Math.max(content.scrollHeight, output.clientHeight) / density));
  ...
  return {
    ...
    destroy() {
      ...
      if (hasNativeHtmlInCanvas) paintable.onpaint = null;
    },
  };
}
      ...
      {!native ? (
        <div
          ...
          style={{
            ...
            width: "100%",
            height: "100%",
            opacity: options.progress !== undefined && options.progress > 0.05 ? Math.max(1 - options.progress * 2, 0) : 1,
