...
const DISPLAY_FRAG = /* glsl */ `
  ...
  void main() {
    ...
    float dyeLen = length(dye.rg);
    
    // When there is no fluid dye (default static state), render Image 100% clean
    if (dyeLen < 0.001) {
      gl_FragColor = vec4(texture2D(uImage, coverUV(vUv, uImageResolution)).rgb, 1.0);
      return;
    }

    vec2 distortion = dye.rg * uDisplacement;
    float mask = clamp(dyeLen * 1.5, 0.0, 1.0);

    // Apply fluid distortion ripple strictly on the single specified image (no secondary image layer)
    vec3 col = texture2D(uImage, coverUV(vUv + distortion, uImageResolution)).rgb;

    // Subtle optical chromatic accent on high-energy fluid swirls
    col += abs(vec3(dye.r, dye.g * 0.6, -dye.r)) * 0.08 * mask;
...
export const FluidImage = ({
  ...
  useEffect(() => {
    ...
    const handlePointerMove = (clientX: number, clientY: number) => {
      ...
      // Check if within bounds of the image container
      if (
        clientX < rect.left ||
        clientX > rect.right ||
        clientY < rect.top ||
      ...
      if (pointer) {
        const dx = (x - pointer.x) * 1.5;
        const dy = (y - pointer.y) * 1.5;
        if (Math.hypot(dx, dy) > 0.001) {
          queue.push({ x, y, dx, dy });
        }
      } else {
        // Initial entry splat
        queue.push({ x, y, dx: 0.05, dy: 0.05 });
      }
      pointer = { x, y };
    };
