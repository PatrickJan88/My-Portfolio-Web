...
export interface DustTextRevealProps {
  text?: string;
  color?: string;
  font?: {
    fontFamily?: string;
...
const renderCanvas = ({
  ...
  const fontSize = parseInt(props.font?.fontSize?.replace("px", "") || "24");
  const font = `${props.font?.fontWeight ?? 500} ${fontSize * globalDpr}px ${formattedFontFamily}`;
...
const useResizeObserver = ({
  ...
  }, [wrapperRef, canvasRef, globalDpr, props, wrapperSize, particlesRef, transformedDensity, color, setWrapperSize]);
};

const parseFramerColor = (color: any): string => {
  if (typeof color !== "string") return "rgba(255, 255, 255, 1)";
  ...
  if (color.startsWith("#")) {
    ...
    return `rgba(${(num >> 16) & 255}, ${(num >> 8) & 255}, ${num & 255}, 1)`;
  }
...
const renderAnimatedTag = (props: any, color: any, showText: any) => {
  const Component: any = (motion as any)[props.tag || "h2"] || motion.h2;
  const trDur =
...
export function DustTextReveal({
  text = "PRODUCT DESIGNER & AI BUILDER",
  color: colorProp = "rgba(220, 226, 240, 0.95)",
  font = {
  ...
  const { wrapperSize, setWrapperSize, wrapperRef } = useWrapperSize();
  ...
  const tranformedCanvasY = useMemo(() => {
    return transformValue(
      parseInt(props.font?.fontSize?.replace("px", "") || "24"),
      [0, 100],
      [0, 5],
      true
    );
  ...
  }, [props.playMode, props.startAlign, props.replay, wrapperRef]);
