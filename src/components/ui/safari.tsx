...
export function Safari({
  ...
  const isWebUrl = !hasVideo && !imageSrc && !children && !!url && (url.startsWith("http://") || url.startsWith("https://"))
  const hasMedia = hasVideo || !!imageSrc || !!children || isWebUrl

  return (
    <div
      className={`relative inline-block w-full align-middle leading-none rounded-[12px] overflow-hidden ${className ?? ""}`}
      style={{
    ...
    >
      ...
      {isWebUrl && (
        <div
          className="pointer-events-none absolute z-0 overflow-hidden bg-[#0A0D14]"
          style={{
            left: `${LEFT_PCT}%`,
            top: `${TOP_PCT}%`,
            width: `${WIDTH_PCT}%`,
        ...
        >
          {/* Ambient dashboard preview while iframe loads */}
          <div className="absolute inset-0 flex flex-col items-center justify-center p-3 bg-gradient-to-b from-[#111420] to-[#0A0D14] text-neutral-400 select-none">
            <div className="flex items-center gap-2 mb-2">
              <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
              <span className="font-mono text-[11px] tracking-wider text-neutral-300 font-medium uppercase">Company 360°</span>
            </div>
            <div className="w-28 h-2 bg-white/10 rounded-full mb-1.5" />
          ...
          <iframe
            ...
            className="relative z-10 block size-full border-0 pointer-events-none select-none bg-transparent"
            loading="lazy"
          />
        </div>
      )}
